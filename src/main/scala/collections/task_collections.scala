...
object task_collections {
  ...
  def capitalizeIgnoringASCII(text: List[String]): List[String] = {
    val textWithIndex = text.zipWithIndex
    textWithIndex.collect{
      case (a, b) if b == 0 => a
      case (a, _) if isASCIIString(a) => a.toUpperCase
      case (a, _) if !isASCIIString(a) => a.toLowerCase
    }
  }
  ...
  def numbersToNumericString(text: String): String = {
    val mapNumber = Map(('0', "zero"),
      ...
      ('8', "eight"),
      ('9', "nine"))
    text.map[String](x => if (mapNumber.contains(x)) mapNumber(x) else x.toString).foldLeft("")((acc, c) => acc.concat(c))
  }
